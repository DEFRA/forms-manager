...
describe('form-definition-repository', () => {
  ...
  describe('updatePageFields', () => {
    ...
    it('should use V1 schema for V1 engine forms', async () => {
      const v1Draft = { ...draft, engine: Engine.V1 }
      mockCollection.findOne.mockReturnValue({ draft: v1Draft })
      mockCollection.findOneAndUpdate.mockResolvedValue({ draft: v1Draft })

      const pageFields = { title: 'Updated title' }
      ...
      expect(updateDoc?.$set?.draft).toBeDefined()
    })

    it('should use V2 schema for V2 engine forms', async () => {
      const v2Draft = { ...draft, engine: Engine.V2 }
      mockCollection.findOne.mockReturnValue({ draft: v2Draft })
      mockCollection.findOneAndUpdate.mockResolvedValue({ draft: v2Draft })
    ...
    it('should call get to retrieve form engine before updating', async () => {
      mockCollection.findOne.mockReturnValue({ draft })
      mockCollection.findOneAndUpdate.mockResolvedValue({ draft })
      ...
      const [firstFilter, firstOptions] = mockCollection.findOne.mock.calls[0]
      expect(firstFilter).toEqual({ _id: new ObjectId(formId) })
    ...
    it('should update page condition field for V2 forms', async () => {
      const v2Draft = { ...draft, engine: Engine.V2 }
      mockCollection.findOne.mockReturnValue({ draft: v2Draft })
    ...
    it('should clear page condition field when set to null', async () => {
      const v2Draft = { ...draft, engine: Engine.V2 }
      mockCollection.findOne.mockReturnValue({ draft: v2Draft })
