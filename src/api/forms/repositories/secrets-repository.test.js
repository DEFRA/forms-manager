...
describe('secrets-repository', () => {
  ...
  describe('get', () => {
    ...
    it('should fail if secret not found', async () => {
      ...
      ).rejects.toThrow(`Form secret 'my-secret' on form ID '${formId}' not found`)
    })
    ...
    it('should throw if db error', async () => {
      ...
      ).rejects.toThrow('DB error get')
      expect(mockLoggerError).toHaveBeenCalledWith(expect.anything(), '[getSecret] Getting form secret \'my-secret\' with form ID fe339c6a-1f6e-4ab8-88c6-73fa1528dc90 failed - DB error get')
    })
  })
  ...
  describe('exists', () => {
    ...
    it('should throw if db error', async () => {
      ...
      expect(mockLoggerError).toHaveBeenCalledWith(expect.anything(), '[existsSecret] Checking existence of form secret \'my-secret\' with form ID fe339c6a-1f6e-4ab8-88c6-73fa1528dc90 failed - DB error exists')
    })
  })
  ...
  describe('save', () => {
    ...
    it('should throw if db error', async () => {
      ...
      expect(mockLoggerError).toHaveBeenCalledWith(expect.anything(), '[mongoError] Secret with name \'my-secret\' for form ID fe339c6a-1f6e-4ab8-88c6-73fa1528dc90 failed to save - MongoDB error code: undefined - DB error insert')
    })
    ...
    it('should throw if other error', async () => {
      ...
      expect(mockLoggerError).toHaveBeenCalledWith(expect.anything(), '[updateError] Secret with name \'my-secret\' for form ID fe339c6a-1f6e-4ab8-88c6-73fa1528dc90 failed to save - DB error insert')
    })
  })
})
